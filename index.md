...
このように、複数の型がありえるメンバーに有効なのがUnionです。
...
例として、Railsで組んだWeb API実装する場合を考えます。
...
こんなプログラムで怖いのは、typoによるミスではないでしょうか。
...
jsonの部分をyamlに変えることでyaml形式にも対応できます。
...
既存の設定ファイルから型定義をつくる場合も、yamlのgeneratorを使うことで作りやすくなります。
